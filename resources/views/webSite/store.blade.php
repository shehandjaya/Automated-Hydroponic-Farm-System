...
<section class="py-5">
    <div class="container px-4 px-lg-5 mt-2">
        <div class="row">
            @php $itemsPerRow = 4; $itemCount = 0; @endphp
        ...
        <div class="row">
            ...
            <div class="col-md-3 col-sm-6 my-3 my-md-0">
                {{-- <form method="POST" action="{{ route('cart.addToCart', ['item_id' => $item->item_id]) }}"> --}}
                    <form action="">
                    @csrf
                    <div class="card shadow position-relative">
                        <div>
                            <img src="{{ $item->image }}" alt="Image1" class="card-img-top"
                        ...
                        <div class="card-body">
                            <h5 class="card-title text-center">{{ Illuminate\Support\Str::limit($item->name, 18) }}</h5>
                            ...
                            <h5 class="d-flex justify-content-center"><small>
                                    <s class="text-secondary text-center">Rs. {{ $item->price }}</s></small></h5>
                            @else
                            <h5 class="d-flex justify-content-center"><small>
                            ...
                            @endphp
                            <h5 class="price text-center text-success">Rs. {{ $finalPrice }}</h5>
                            ...
                            <div class="store-overlay d-flex justify-content-center align-items-center">
                                <button type="button" class="btn btn-primary rounded-pill py-2 px-4 m-2"
                                    onclick="addToCart('{{ $item->item_id }}', '{{ $item->name }}', {{ $item->price }}, {{ $item->discount }}, '{{ $item->description }}')">Add
...
<script type="text/javascript">
    $(document).on('keyup', '#search', function () {
        ...
        $.ajax({
            type: 'get',
            url: '{{ route('search') }}', // Use the named route
            data: {
                'search': $value
            },
            success: function (data) {
